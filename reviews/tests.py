...
class ReviewListViewTests(APITestCase):
    ...
    def test_can_list_reviews(self):
        ...
        Review.objects.create(
            owner=ben, movie=movie, content='a review', rating=3)
    ...
    def test_logged_in_user_can_create_review(self):
        ...
        response = self.client.post(
            '/reviews/', {
                'movie': [1], 'content': ['a review'], 'rating': [3]})
    ...
    def test_user_not_logged_in_cant_create_review(self):
        movie = Movie.objects.get(title='movie')
        response = self.client.post(
            '/reviews/', {'movie': [1], 'content': ['a review'], 'rating': 3})
...
class ReviewDetailViewTests(APITestCase):
    def setUp(self):
        ...
        Review.objects.create(
            owner=ben, content='bens review',
            movie=Movie.objects.get(title='movie'), rating=3
        )
        Review.objects.create(
            owner=olga, content='olgas review',
            movie=Movie.objects.get(title='movie'), rating=1
        )
    ...
    def test_user_can_update_own_review(self):
        ...
        response = self.client.put(
            '/reviews/1', {
                'movie': [1], 'content': ['bens updated review'], 'rating': 3})
